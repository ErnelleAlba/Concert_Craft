...
function ScheduleBooking() {
  ...
  const handleOnChange = async (e) => {
    ...
    const resTitle = await axios(
      `http://localhost:8000/api/v1/concerts?title=${e.target.value}`
    );

    dispatch(
      setConcerts(
        resTitle.data.data.map((concert) => {
          return {
            id: concert.id,
            title: concert.title,
            posterImageUrl: concert.posterImageUrl.replace(
              "storage",
    ...
    <>
      ...
      <div className="d-flex flex-column">
        <div className="d-flex justify-content-center align-self-end col-10 col-lg-2 col-md-4 col-sm-5 me-5 mb-2 bg-dark text-light px-2 py-1 rounded">
          <label htmlFor="search-concert-bar" className="me-2 pt-2 form-label">
            Search:{" "}
          </label>
          <input
            type="text"
            id="search-concert-bar"
        ...
        <div className="container-fluid mb-5 d-flex flex-wrap justify-content-center">
          {isLoading ? (
            ...
            concertCard.map((concertCard, index) => (
              <ConcertScheduleCard key={index} concertCard={concertCard} />
            ))
          ) : (
