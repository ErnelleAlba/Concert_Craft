...
function BookingCard({ id }) {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ticketItem: Yup.number()
        .integer("Must be a number")
        .positive("Must be positive number")
        .required("Input number of tickets"),
    ...
    onSubmit: async (value) => {
      ...
      try {
        ...
        if (res.status === 201) {
          alert(
            "Your booking is successful check the email we send to you for details. Thank you for booking!!"
          );
        }
      } catch (err) {
  ...
  const fetchConcertInfo = async () => {
    ...
    const concertObj = {
      ...
      posterImageUrl: concertInfo.posterImageUrl.replace(
        "storage",
        ...
        <div className="row no-gutters">
          <div className="col-md-4">
            <img
              ...
              className="img-fluid rounded border border-black w-100"
              alt={concert.title}
            />
          </div>
            ...
            <div className="card-body">
              ...
              <div className="card-text d-flex flex-column gap-1 mb-3">
                <small className="text-muted">
                  ...
                  Place:<span className="ms-1">{concert.eventPlace}</span>
                </small>
                <small className="text-muted">
                  <i className="fa-regular fa-calendar me-1"></i>
                  Date:{" "}
                  <span className="ms-1">
                    {concert.eventDate.split("-")[0]}
                  </span>
                </small>
                <small className="text-muted">
                  ...
                  Time:{" "}
                  <span className="ms-1">
                    {concert.eventDate.split("-")[1]}
                  </span>
                </small>
                <h5 className="card-text text-danger mt-2">
                  <span className="text-dark me-2">Ticket Price: </span>
                  <i className="fa-solid fa-peso-sign me-1"></i>
                  {concert.ticketPrice}
                </h5>
              </div>
              <form
              ...
              >
                <div className="d-flex mb-2 col-md-6 column">
                  <label htmlFor="ticketItem" className="form-label me-3">
                    Number of Tickets:
                  </label>
                  <div className="row col">
                    ...
                    {formik.errors.ticketItem && formik.touched.ticketItem ? (
                      <span className="text-danger">
                        {formik.errors.ticketItem}
                      </span>
                    ) : null}
                  </div>
                </div>
                <div className="mb-2">
                  <label htmlFor="seatPosition" className="me-2">
                    Select an option:
                  </label>
                  <select
                    name="seatPosition"
                ...
                <h4 className="pb-1">
                  Total Amount:{" "}
                  <i className="fa-solid fa-peso-sign text-danger me-1"></i>
                  <span className="text-danger">{totalAmount()}</span>
                </h4>
                <button
                  type="submit"
                  className="btn btn-dark btn-lg btn-block col-md-12 text-light mt-2"
                >
                  <i className="fa-solid fa-ticket me-2"></i> Book your tickets
                </button>
              </form>
